  ...
  return (
    <div className="py-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass rounded-2xl p-4">
          <div className="text-2xl font-bold">{totalRevenue.toLocaleString()} sum</div>
      ...
      {isLoading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass rounded-2xl p-4">
          <h3 className="font-semibold text-sm mb-3">📈 Revenue & Rides</h3>
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={revenue}>
              ...
              <Line yAxisId="right" type="monotone" dataKey="rides" stroke="#0c8ee7" strokeWidth={2} name="Rides" />
            </LineChart>
          </ResponsiveContainer>
        </motion.div>
      ) : (
        <div className="text-center py-8 text-gray-500 text-sm">No data for this period</div>
      )}

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass rounded-2xl p-4">
          <h3 className="font-semibold text-sm mb-3">🏆 Top Drivers</h3>
          {topDrivers.length === 0 ? (
            ...
            <div className="space-y-2">
              {topDrivers.map((d: any, i: number) => (
                <div key={d._id} className="flex items-center justify-between text-xs py-1.5 border-b border-white/5">
                  ...
                  <div className="text-gray-400">{d.rides} rides • {d.revenue?.toLocaleString()} sum</div>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="space-y-2">
              {topCustomers.map((c: any, i: number) => (
                <div key={c._id} className="flex items-center justify-between text-xs py-1.5 border-b border-white/5">
                  ...
                  <div className="text-gray-400">{c.rides} rides • {c.spent?.toLocaleString()} sum</div>
                </div>
              ))}
            </div>
          )}
