...
function SettingsInput({ label, path, type = 'text', step, form, update }: {
  label: string; path: string; type?: string; step?: string;
  form: any; update: (path: string, value: any) => void;
}) {
...
function SettingsToggle({ label, path, form, update }: {
  label: string; path: string; form: any; update: (path: string, value: any) => void;
}) {
  const keys = path.split('.');
  let val: any = form;
  for (const k of keys) val = val?.[k];
    ...
    <div className="space-y-4">
      ...
      <SettingsSection title={t('general')}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <SettingsInput label={t('app_name')} path="general.appName" form={form} update={update} />
          <SettingsInput label={t('support_phone')} path="general.contactPhone" form={form} update={update} />
          <SettingsInput label={t('support_url')} path="general.supportUrl" form={form} update={update} />
          <SettingsInput label={t('default_language')} path="general.defaultLanguage" form={form} update={update} />
        </div>
      </SettingsSection>

      <SettingsSection title={t('pricing')}>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          <SettingsInput label={t('base_fare_sum')} path="pricing.baseFare" type="number" form={form} update={update} />
          <SettingsInput label={t('price_per_km')} path="pricing.pricePerKm" type="number" form={form} update={update} />
          <SettingsInput label={t('price_per_minute')} path="pricing.pricePerMinute" type="number" form={form} update={update} />
          <SettingsInput label={t('minimum_fare')} path="pricing.minimumFare" type="number" form={form} update={update} />
          <SettingsInput label={t('night_coefficient')} path="pricing.nightCoefficient" type="number" step="0.1" form={form} update={update} />
          <SettingsInput label={t('rush_coefficient')} path="pricing.rushCoefficient" type="number" step="0.1" form={form} update={update} />
          <SettingsInput label={t('airport_fee')} path="pricing.airportFee" type="number" form={form} update={update} />
        </div>
        <SettingsToggle label={t('surge_enabled')} path="pricing.surgeEnabled" form={form} update={update} />
      </SettingsSection>

      <SettingsSection title={t('search_settings')}>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          <SettingsInput label={t('max_radius')} path="search.maxRadius" type="number" form={form} update={update} />
          <SettingsInput label={t('search_timeout')} path="search.searchTimeout" type="number" form={form} update={update} />
      ...
      <SettingsSection title={t('driver_settings')}>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <SettingsInput label={t('commission_percent')} path="driver.commission" type="number" form={form} update={update} />
          <SettingsInput label={t('min_rating')} path="driver.minRating" type="number" step="0.1" form={form} update={update} />
          <SettingsInput label={t('max_rides_before_break')} path="driver.maxRidesBeforeBreak" type="number" form={form} update={update} />
        </div>
      </SettingsSection>

      <SettingsSection title={t('features')}>
        <div className="flex flex-wrap gap-x-6 gap-y-2">
          <SettingsToggle label={t('food_delivery')} path="features.foodDelivery" form={form} update={update} />
          <SettingsToggle label={t('ride_scheduling')} path="features.rideScheduling" form={form} update={update} />
          <SettingsToggle label={t('referral_system')} path="features.referralSystem" form={form} update={update} />
          <SettingsToggle label={t('sos_button')} path="features.sosButton" form={form} update={update} />
        </div>
      </SettingsSection>

      <SettingsSection title={t('district_boundary')}>
        <div className="space-y-3">
      ...
      <SettingsSection title={t('maintenance')}>
        <SettingsToggle label={t('maintenance_mode')} path="maintenance.isEnabled" form={form} update={update} />
        {form.maintenance?.isEnabled && <SettingsInput label={t('maintenance_message')} path="maintenance.message" form={form} update={update} />}
      </SettingsSection>
